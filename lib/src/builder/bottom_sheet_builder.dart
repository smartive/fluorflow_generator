    ...
    await for (final assetId in buildStep.findAssets(_allDartFilesInLib)) {
      ...
      for (final (sheetClass, superType) in lib.classes
        ...
        final methodTupleRef = RecordType((b) => b
          ...
          ..positionalFieldTypes.add(switch (sheetReturnType) {
            analyzer.VoidType() ||
            analyzer.DynamicType() =>
              refer(sheetReturnType.getDisplayString(withNullability: false)),
            _ => TypeReference((b) => b
              ..isNullable = true
              ..symbol =
                  sheetReturnType.getDisplayString(withNullability: false)
              ..url = lib.pathToElement(sheetReturnType.element!).toString()),
          }));
