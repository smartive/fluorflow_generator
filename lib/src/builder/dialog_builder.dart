    ...
    await for (final assetId in buildStep.findAssets(_allDartFilesInLib)) {
      ...
      for (final (dialogClass, superType) in lib.classes
        ...
        final methodTupleRef = RecordType((b) => b
          ...
          ..positionalFieldTypes.add(switch (dialogReturnType) {
            analyzer.VoidType() ||
            analyzer.DynamicType() =>
              refer(dialogReturnType.getDisplayString(withNullability: false)),
            _ => TypeReference((b) => b
              ..isNullable = true
              ..symbol =
                  dialogReturnType.getDisplayString(withNullability: false)
              ..url = lib.pathToElement(dialogReturnType.element!).toString()),
          }));
